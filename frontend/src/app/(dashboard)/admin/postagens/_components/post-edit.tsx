...
export function PostEditScreen({ onClose, post }: PostEditScreenProps) {
  ...
  async function submit(form: FormData) {
    ...
    const { error } = JSON.parse(await updatePost(newForm)) as {
    ...
    if (error) {
      setError(error)
      toast({
        title: 'Nao foi possivel editar o post!',
      })
    } else {
      toast({
    ...
    <section className="absolute inset-0 z-20 overflow-y-auto bg-zinc-100 px-6 py-8 text-zinc-950">
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Button className="bg-zinc-700 hover:bg-zinc-800" onClick={onClose}>
            Cancelar
          </Button>
        </div>
